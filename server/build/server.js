...
const { ValidateCellOutput, ValidateOutPoint, ValidateScript } = ckbJsToolkit.validators;
...
const { ValidateOutPoint: ValidateOutPoint$1, ValidateScript: ValidateScript$1 } = ckbJsToolkit.validators;
const { NormalizeScript } = ckbJsToolkit.normalizers;
...
function serializeOutPoint(outPoint) {
  if (outPoint instanceof Object) {
    ValidateOutPoint$1(outPoint);
    return outPoint.tx_hash + outPoint.index;
  } else {
    return new ckbJsToolkit.Reader(outPoint).serializeJson();
  }
}
...
function calculateScriptHash(validatedScript) {
  const h = blake2b(
    32,
    null,
    null,
    new Uint8Array(ckbJsToolkit.Reader.fromRawString("ckb-default-hash").toArrayBuffer())
  );
  const data = SerializeScript(NormalizeScript(validatedScript));
  h.update(new Uint8Array(data));
  const out = new Uint8Array(32);
  h.digest(out);
  return new ckbJsToolkit.Reader(out.buffer).serializeJson();
}

class LiveCellClass extends nohm.NohmModel {
  setCell(cell) {
...
const LiveCell = nohm.Nohm.register(LiveCellClass);
...
class Indexer {
  ...
  start() {
    if (this.registerNohm) {
      nohm.Nohm.setClient(this.redisClient);
    }
    const timeout = 1000;
    const f = () => {
      this.loop()
  ...
  async loop() {
    const getAsync = util.promisify(this.redisClient.get).bind(this.redisClient);
    const setAsync = util.promisify(this.redisClient.set).bind(this.redisClient);
    const delAsync = util.promisify(this.redisClient.del).bind(this.redisClient);
    ...
    while (true) {
      ...
      const block = await this.rpc.get_block_by_number(
        ckbJsToolkit.BigIntToHexString(blockNumber)
      );
      if (!block) {
      ...
      if (JSBI.greaterThan(blockNumber, JSBI.BigInt(0))) {
        const previousIndexedBlockHash = await getAsync(
          `BLOCK:${ckbJsToolkit.BigIntToHexString(lastProcessedBlockNumber)}:HASH`
        );
        if (previousIndexedBlockHash !== block.header.parent_hash) {
          ...
          if (lastUnpurgedBlockNumber) {
            if (
              JSBI.lessThan(
                ...
                ckbJsToolkit.HexStringToBigInt(lastUnpurgedBlockNumber)
              )
            ) {
              throw new Error(
          ...
          await setAsync(
            ...
            ckbJsToolkit.BigIntToHexString(JSBI.add(blockNumber, JSBI.BigInt(-1)))
          );
          continue;
        }
      }
        ...
        for (let i = 0; i < transaction.outputs.length; i++) {
          cells.push({
            ...
            out_point: {
              ...
              index: ckbJsToolkit.BigIntToHexString(JSBI.BigInt(i))
            },
      ...
      for (const cell of cells) {
        const c = await nohm.Nohm.factory(KEY_LIVE_CELL);
        const ids = await LiveCell.find({
        ...
        c.property(
          KEY_BLOCK_NUMBER,
          JSBI.toNumber(ckbJsToolkit.HexStringToBigInt(block.header.number))
        );
        await c.save();
      }
      ...
      await setAsync(
        `BLOCK:${ckbJsToolkit.BigIntToHexString(blockNumber)}:HASH`,
        block.header.hash
      );
      ...
      if (this.purgeOldBlocks) {
        ...
        if (JSBI.greaterThanOrEqual(blockToPurge, JSBI.BigInt(0))) {
          const blockHash = await getAsync(
            `BLOCK:${ckbJsToolkit.BigIntToHexString(blockToPurge)}:HASH`
          );
          await setAsync(
            "LAST_UNPURGED_BLOCK_NUMBER",
          ...
          await delAsync(`BLOCK:${ckbJsToolkit.BigIntToHexString(blockToPurge)}:HASH`);
        }
      }
    }
  }
